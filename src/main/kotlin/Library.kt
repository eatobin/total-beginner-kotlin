...
object Library {
    ...
    fun <A> addItem(x: A, xs: List<A>): List<A> {
        return if (xs.contains(x)) xs
        else xs.plusElement(x)
    }

    fun removeBook(bk: Book, bks: Books): Books = bks.filter { it != bk }
...
//    fun getBooksForBorrower(br: Borrower, bks: Books): Books = bks.filter { it.borrower == br }
    ...
    private fun notMaxedOut(br: Borrower, bks: Books): Boolean = numBooksOut(br, bks) < getMaxBooks(br)

    private fun bookNotOut(bk: Book): Boolean = getBorrower(bk) == null

    private fun bookOut(bk: Book): Boolean = getBorrower(bk) != null
    ...
    private fun libraryToString(bks: Books, brs: Borrowers): String {
        return "Test Library: " + bks.count() + " books; " + brs.count() + " borrowers."
    }
            ...
            try {
                reader.beginArray {
                    while (reader.hasNext()) {
                        ...
                        if (borrower != null) result.add(borrower)
                    }
                }
            } catch (_: Exception) {
